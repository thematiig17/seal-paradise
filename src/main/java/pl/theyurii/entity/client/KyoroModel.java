...
public class KyoroModel<T extends KyoroEntity> extends SinglePartEntityModel<T> {

    public static final EntityModelLayer KYORO = new EntityModelLayer(Identifier.of(SealParadise.MOD_ID, "kyoro"),"main");


    private final ModelPart foka;
    private final ModelPart gowa;
    private final ModelPart PYSZCZEK;
    private final ModelPart zrenice;
    private final ModelPart Lewa;
    private final ModelPart Prawa;
    private final ModelPart warga;
    ...
    public KyoroModel(ModelPart root) {
        this.foka = root.getChild("foka");
        this.gowa = this.foka.getChild("gowa");
        this.PYSZCZEK = this.gowa.getChild("PYSZCZEK");
        this.zrenice = this.gowa.getChild("zrenice");
        this.Lewa = this.zrenice.getChild("Lewa");
        this.Prawa = this.zrenice.getChild("Prawa");
        this.warga = this.gowa.getChild("warga");
    ...
    public static TexturedModelData getTexturedModelData() {
        ...
        ModelPartData PYSZCZEK = gowa.addChild("PYSZCZEK", ModelPartBuilder.create().uv(24, 29).cuboid(8.0F, -2.0F, -2.0F, 2.0F, 1.0F, 3.0F, new Dilation(0.0F)), ModelTransform.pivot(-2.0F, 0.0F, 0.0F));

        ModelPartData zrenice = gowa.addChild("zrenice", ModelPartBuilder.create(), ModelTransform.pivot(-1.0F, 0.0F, 0.0F));

        ModelPartData Lewa = zrenice.addChild("Lewa", ModelPartBuilder.create().uv(4, 36).cuboid(5.0F, -3.0F, -2.0F, 2.0F, 1.0F, 1.0F, new Dilation(0.0F)), ModelTransform.pivot(0.0F, 0.0F, 0.0F));

        ModelPartData Prawa = zrenice.addChild("Prawa", ModelPartBuilder.create().uv(4, 36).cuboid(5.0F, -3.0F, 0.0F, 2.0F, 1.0F, 1.0F, new Dilation(0.0F)), ModelTransform.pivot(0.0F, 0.0F, 0.0F));

        ModelPartData warga = gowa.addChild("warga", ModelPartBuilder.create().uv(25, 2).cuboid(8.0F, -1.0F, -2.0F, 2.0F, 1.0F, 3.0F, new Dilation(0.0F)), ModelTransform.pivot(-2.0F, 0.0F, 0.0F));
        ...
        ModelPartData tylniaapa2_r1 = apatylnia2.addChild("tylniaapa2_r1", ModelPartBuilder.create().uv(0, 27).cuboid(-0.766F, 0.0F, -2.3572F, 2.0F, 1.0F, 4.0F, new Dilation(0.0F)), ModelTransform.of(-13.0F, -1.0F, -3.0F, 0.0F, -2.2689F, 0.0F));

        ModelPartData ogonprzytuowiu = foka.addChild("ogonprzytuowiu", ModelPartBuilder.create().uv(0, 11).cuboid(-9.0F, -4.0F, -3.0F, 6.0F, 4.0F, 5.0F, new Dilation(0.0F)), ModelTransform.pivot(0.0F, 0.0F, 0.0F));

        ModelPartData ogon = foka.addChild("ogon", ModelPartBuilder.create().uv(12, 29).cuboid(-12.0F, -3.0F, -2.0F, 3.0F, 3.0F, 3.0F, new Dilation(0.0F)), ModelTransform.pivot(0.0F, 0.0F, 0.0F));
    ...
    @Override
    public void setAngles(KyoroEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.getPart().traverse().forEach(ModelPart::resetTransform);
        this.setHeadAngles(netHeadYaw, headPitch);

        this.animateMovement(KyoroAnimations.ANIM_KYORO_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
        this.updateAnimation(entity.idleAnimationState, KyoroAnimations.ANIM_KYORO_IDLE, ageInTicks, 1f);
    }
